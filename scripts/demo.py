...
def load_txt(gt_path):
    ...
    try:
        ...
        for fid, line in enumerate(gt):
            index,x, y, w, h = map(float, line.split(','))
...
def main(args):
    try:
        ...
        with torch.inference_mode():
            ...
            for target_index, bbox in prompts.items():
                _, _, masks = predictor.add_new_points_or_box(state, box=bbox, frame_idx=0, obj_id=target_index)
            ...
            for frame_idx, object_ids, masks in tqdm(
                ...
                # 创建结果图像（默认使用原始图像）
                result = frame.copy()
                foreground = [np.full_like(frame, (0, 0, 0)) for _ in range(args.target_num)]  # 黑色背景
                ...
                for obj_id, mask in zip(object_ids, masks):
                    ...
                    if np.any(mask):
                        # 前景：只保留mask区域的原始图像
                        np.copyto(foreground[obj_id], frame, where=mask[:, :, None])
                        # 背景：将mask区域设置为绿色
                        background[mask] = (0, 255, 0)     
                        if info_text == "":
                            info_text = f'Frame: {frame_idx} {obj_id}'               
                        else:
                            info_text += f'/{obj_id}'
                    else:
                        # 添加帧信息
                        if info_text == "":
                            info_text = f'Frame: {frame_idx} {obj_id} no mask'
                        else:
                            info_text += f'/{obj_id} no mask'
                ...
                combined_foreground = np.zeros_like(foreground[0])
                for fg in foreground:
                    combined_foreground += fg
                cv2.imshow('Foreground', combined_foreground)
                ...
                # 保存前景和背景图像
                    for i, fg in enumerate(foreground):
                        save_frame_image(
                            frame=fg,
                            output_dir=args.output_fg_path + f"/{i}",
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_video_path", default="D:/2024Work/14.AIVideo/6.Assets/6.Video/all/", help="Input video path.")
    parser.add_argument("--force_cpu", default=True, type=bool, help="强制使用CPU运行，忽略GPU")
